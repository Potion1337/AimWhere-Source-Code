...
class Velocity : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf("Normal", "AAC", "AAC4.4.2","AACPush", "AACZero",
            "Reverse", "SmoothReverse", "Jump", "Glitch"), "Normal")
    ...
    override val tag: String
        get() = if(modeValue.get() == "Normal")
            "${horizontalValue.value}% ${verticalValue.value}%"
        else
            modeValue.get()
            ...
            "aac4.4.2" -> {
                if (mc.thePlayer.hurtTime != 0 && damaged) {
                    if (!mc.thePlayer.isBurning) {
                        ...
                        if (mc.thePlayer.onGround) {
                            mc.thePlayer.motionX = (mc.thePlayer.motionX * 0.6F)
                            mc.thePlayer.motionZ = (mc.thePlayer.motionZ * 0.6F)
                        } else {
                            if (velocityTimer.hasTimePassed(80L)) {
                                if (mc.thePlayer.ticksExisted % 3 == 0)
                                    mc.thePlayer.motionY = (mc.thePlayer.motionY - 0.0169 * mc.thePlayer.fallDistance)
                                damaged = false
                                velocityTimer.reset()
                            }
                            mc.thePlayer.motionX = (mc.thePlayer.motionX * 0.6F)
