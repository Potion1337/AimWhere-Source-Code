...
@ModuleInfo(
        name = "ChestStealer",
        description = "Open the chest to get items.",
        category = ModuleCategory.WORLD,
        keyBind = Keyboard.KEY_X
)
class ChestStealer : Module() {
    private val delay = IntegerValue("Delay", 150, 0, 1000)
    val silentValue = BoolValue("Silent", true)
    private val menuCheckValue = BoolValue("MenuCheck", true)
    ...
    private fun onTick(event: TickEvent) {
        if (mc.thePlayer != null && mc.thePlayer.openContainer is ContainerChest) {
            ...
            if(menuCheckValue.get()&&(chestName.contains("menu") || chestName.contains("play") || chestName.contains("game") || chestName.contains("菜单") || chestName.contains("信息")))
                return
            ...
            while (i < container.lowerChestInventory.sizeInventory) {
                if (container.lowerChestInventory.getStackInSlot(i) != null && timer.hasReached(delay.get().toDouble())
