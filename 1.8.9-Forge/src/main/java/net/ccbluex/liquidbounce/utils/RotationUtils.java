...
public class RotationUtils extends MinecraftInstance implements Listenable {
     ...
     * @return
     */
    public static Rotation getHypixelRotationsNeededBlock(double x2, double y2, double z2) {
        double diffX = x2 + 0.5 - mc.thePlayer.posX;
        double diffZ = z2 + 0.5 - mc.thePlayer.posZ;
        double diffY = y2 - mc.thePlayer.posY - 0.2;
        double dist = MathHelper.sqrt_double(diffX * diffX + diffZ * diffZ);
        float yaw = (float)(Math.atan2(diffZ, diffX) * 180.0 / 3.141592653589793) - 90.0f;
        float pitch = (float)(-Math.atan2(diffY, dist) * 180.0 / 3.141592653589793);
        float[] arrf = new float[2];
        arrf[0] = mc.thePlayer.rotationYaw + MathHelper.wrapAngleTo180_float(yaw - (float)(120 + new Random().nextInt(2)));
        arrf[1] = mc.thePlayer.rotationPitch + MathHelper.wrapAngleTo180_float(pitch - mc.thePlayer.rotationPitch);
        return new Rotation(arrf[0],arrf[1]);
    }

    public static Rotation getHypixelRotations(Entity entityIn) {
        double x = entityIn.posX;
        double z = entityIn.posZ;
        double y = entityIn.posY + (entityIn.getEyeHeight() / 2.0);
        return getHypixelRotationsNeededBlock(x,y,z);
    }
    ...
    public static Rotation getVecRotation(Vec3 origin, Vec3 position) {
