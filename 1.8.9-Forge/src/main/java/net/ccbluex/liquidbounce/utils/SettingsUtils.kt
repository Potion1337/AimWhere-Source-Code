...
import me.kiras.aimwhere.modules.player.AutoAbuse
            ...
            when (args[0]) {
                ...
                "load" -> {
                    ...
                    val url = if (urlRaw.startsWith("http"))
                        ...
                        "${LiquidBounce.CLIENT_CLOUD}/ModueSettings/${urlRaw.toLowerCase()}"

                    try {
                        ClientUtils.displayChatMessage("§7[§3§lAutoSettings§7] §7Loading ModueSettings from §a§l$url§7...")
                        executeScript(get(url))
                        ClientUtils.displayChatMessage("§7[§3§lAutoSettings§7] §7Loaded ModueSettings from §a§l$url§7.")
                    } catch (e: Exception) {
                        ClientUtils.displayChatMessage("§7[§3§lAutoSettings§7] §7Failed to load ModueSettings from §a§l$url§7.")
                    }
                }
