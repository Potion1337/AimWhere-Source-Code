...
object LoginUtils : MinecraftInstance() {
    ...
    fun randomCracked(){
        try {
            loginCracked(nameBase[(Math.random() * nameBase.size).roundToInt()] + "_" + RandomUtils.nextInt(0, 999))
        } catch(exception: ArrayIndexOutOfBoundsException) {
            loginCracked(nameBase[(Math.random() * (nameBase.size - 50)).roundToInt()] + "_" + RandomUtils.nextInt(0, 999))
        }
    }
