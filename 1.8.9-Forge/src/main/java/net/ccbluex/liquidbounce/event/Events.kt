...
class ModelUpdateEvent(val player: EntityPlayer, val modelPlayer: ModelPlayer): Event()
/**
 * Called when player attacks other entity
 *
 * @param targetEntity Attacked entity
 */
class BlockRenderEvent(val x : Float,val y : Float,val z : Float,val block: Block) : Event()
...
class BlockRenderSideEvent(var world: IBlockAccess, var pos: BlockPos, var side: EnumFacing, var maxX: Double, var minX: Double, var maxY: Double, var minY: Double, var maxZ: Double, var minZ: Double) : CancellableEvent() {
